# Main evaluation script for MambaLM on TinyStories-style LM dataset.
# Logs:
#   - LM eval_loss, eval_ppl (perplexity), token count
#   - sample-based text stats: len_tokens, distinct_2/3, rep_2/3
#   - optional reference-based metrics (if enabled + deps installed + refs provided):
...
python -m pocket_narrator.models.mamba.mamba_evaluation \
  --model_config configs/mamba_tinystories_3M/model.yaml \
  --tokenizer_config configs/mamba_tinystories_3M/tokenizer.yaml \
  --checkpoint results/mamba_tinystories_3M/mamba_best.pt \
  --num_workers 0

python -m pocket_narrator.models.mamba.mamba_evaluation \
  --model_config configs/mamba_tinystories_18M/model.yaml \
  --tokenizer_config configs/mamba_tinystories_18M/tokenizer.yaml \
  --checkpoint results/mamba_tinystories_18M/mamba_best.pt \
  --num_workers 0       
"""
...
# Optional metrics (only used if installed + enabled)
try:
    import sacrebleu
except ImportError:
    sacrebleu = None
...
def compute_reference_metrics(
    ...
    references: List[str],
    bertscr_model_type: Optional[str] = None,
) -> Dict[str, float]:
    """
    Compute reference-based metrics for generation quality.

    Requires:
      pip install sacrebleu rouge-score bert-score

    Notes:
      - BLEU/ROUGE/BERTScore require aligned references (one per prompt).
      - These metrics can be slow (especially BERTScore).
    """
    missing = []
    ...
    if missing:
        raise ImportError(
            "Missing optional deps for reference metrics: "
            + ", ".join(missing)
            + ". Install via: pip install sacrebleu rouge-score bert-score"
        )

    if len(generations) != len(references):
        raise ValueError(
            f"generations and references must have same length, got {len(generations)} vs {len(references)}."
        )

    # BLEU (corpus BLEU)
    bleu = sacrebleu.corpus_bleu(generations, [references])

    # ROUGE-L (mean F1 over pairs)
    scorer = rouge_scorer.RougeScorer(["rougeL"], use_stemmer=True)
    rouge_l_scores = [
        scorer.score(ref, gen)["rougeL"].fmeasure
        for ref, gen in zip(references, generations)
    ]
    rouge_l = float(sum(rouge_l_scores) / max(len(rouge_l_scores), 1))
    ...
    P, R, F1 = bert_score(
        ...
        lang="en",
        verbose=False,
        model_type=bertscr_model_type,
    )

    # P/R/F1 are torch tensors
    return {
...
def generate_samples(
    ...
    # fallback: if eos is missing, use pad as stop token (common in toy tokenizers)
...
def evaluate_lm_loss_ppl(
    ...
    for batch in dataloader:
        ...
        # Token-aware aggregate: count non-pad tokens
...
def _build_mcfg_from_sources(
    ...
    """
    if (
        ckpt_payload is not None
        and "config" in ckpt_payload
        and isinstance(ckpt_payload["config"], dict)
    ):
...
def main() -> None:
    ...
    parser.add_argument("--batch_size", type=int, default=None, help="Override eval batch_size")
    ...
    parser.add_argument("--refs_file", type=str, default=None,
                        help="Optional txt file with one reference per line (must align with prompts).")
    parser.add_argument("--enable_ref_metrics", action="store_true",
                        help="Compute BLEU/ROUGE-L/BERTScore (requires deps + refs_file).")
    parser.add_argument("--bertscore_model", type=str, default=None,
                        help="Optional BERTScore model_type override (e.g., 'roberta-large').")
    ...
    bs = args.batch_size if args.batch_size is not None else 1  # keep default simple/safe
    ...
    if args.refs_file:
        rp = Path(args.refs_file)
        references = [line.strip() for line in rp.read_text(encoding="utf-8").splitlines() if line.strip()]
    ...
    if not args.no_wandb:
        run = wandb.init(
            ...
            config={
                ...
                "num_params": n_params,
                "enable_ref_metrics": bool(args.enable_ref_metrics),
    ...
    print(
        f"[RESULT] eval_loss={lm_metrics['eval_loss']:.4f} "
        f"eval_ppl={lm_metrics['eval_ppl']:.2f} "
        f"tokens={int(lm_metrics['eval_tokens'])}"
    )
    ...
    if args.enable_ref_metrics:
        if references is None:
            print("[WARN] --enable_ref_metrics set but --refs_file not provided. Skipping BLEU/ROUGE/BERTScore.")
        else:
            if len(references) != len(prompts):
                raise ValueError(
                    f"refs_file must contain exactly one reference per prompt. "
                    f"Got {len(references)} refs for {len(prompts)} prompts."
                )
            generations = [s["generation"] for s in samples]
            ref_metrics = compute_reference_metrics(
                generations=generations,
                references=references,
                bertscr_model_type=args.bertscore_model,
            )
            print(
                "[RESULT] "
                f"BLEU={ref_metrics['bleu']:.2f} "
                f"ROUGE-L={ref_metrics['rougeL']:.4f} "
                f"BERTScore(F1)={ref_metrics['bertscore_f1']:.4f}"
            )
